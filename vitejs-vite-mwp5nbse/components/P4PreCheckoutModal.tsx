...
const DOCS = {
  ...
  WIDERRUF: {
    ...
    content: 'Текст Widerrufsbelehrung...',
  },
  MANDAT: {
    title: 'Mandatsvereinbarung',
    content: 'Текст Mandatsvereinbarung...',
  },
  RVG: {
    title: 'Honorarvereinbarung',
    content: 'Текст Honorarvereinbarung...',
  },
  EXECUTION: {
    title: 'Sofortige Aufnahme der Tätigkeit',
    content: 'Текст поручения о немедленном начале...',
  },
  PRIVACY: {
...
const registerAndCheckStatus = async (
  ...
  try {
    ...
    if (
      (statusP1 === 'PURCHASED' || statusP1 === 'COMPLETED') &&
      diagnosis === 'POSITIVE'
    )
...
const modalContent = {
  ru: {
    ...
    family: {
      ...
      children: 'Дети',
      info: 'Базовый семейный формат включает до 3 человек. Каждый последующий участник рассчитывается отдельно.',
    },
    credits: {
      title: 'РАНЕЕ ПРИОБРЕТЁННЫЕ ПАКЕТЫ (ЗАЧЁТ)',
      p1_label: 'Пакет №1 «Стартовый»',
      p2_label: 'Пакет №2 «Самостоятельный»',
      p3_label: 'Пакет №3 «Адвокатский»',
      p1_summary: 'Зачет пакета №1',
      p2_summary: 'Зачет пакета №2',
      p3_summary: 'Зачет пакета №3',
    },
    summary: {
      base: 'БАЗОВАЯ ЦЕНА ПАКЕТА №4',
    ...
    offer_scope:
      'Это предложение предназначено исключительно для потребителей в понимании § 13 BGB.',
    p1_precondition:
      'Бронирование этого пакета возможно только при наличии положительного результата в рамках Пакета №1.',
    privacy_link: 'Политика конфиденциальности',
    button: 'ПРОВЕРИТЬ И ПРОДОЛЖИТЬ',
    loading: 'СОЗДАНИЕ ЗАКАЗА...',
    package_name_order: 'Пакет №4 «Премиум»',
    errors: {
      not_found: 'Нужен положительный результат Пакета №1.',
      pending: 'Оценка еще не завершена.',
      negative: 'Результат не подтвержден.',
  ...
  de: {
    ...
    family: {
      ...
      spouse: 'Ehepartner',
      children: 'Kinder',
      info: 'Das Standard-Familienformat umfasst bis zu 3 Personen. Jeder weitere Teilnehmer wird separat berechnet.',
    },
    credits: {
      title: 'BEREITS ERWORBENE PAKETE (VERRECHNUNG)',
      p1_label: 'Paket №1 «Start»',
      p2_label: 'Paket №2 «Selbstständig»',
      p3_label: 'Paket №3 «Anwaltlich»',
      p1_summary: 'Anrechnung Paket №1',
      p2_summary: 'Anrechnung Paket №2',
      p3_summary: 'Anrechnung Paket №3',
    },
    summary: {
      base: 'BASISPREIS PAKET №4',
    ...
    offer_scope:
      'Dieses Angebot richtet sich ausschließlich an Verbraucher im Sinne des § 13 BGB.',
    p1_precondition:
      'Die Buchung dieses Pakets ist nur möglich, wenn im Rahmen von Paket 1 ein positives Ergebnis festgestellt wurde.',
    privacy_link: 'Datenschutzerklärung',
    button: 'PRÜFEN UND FORTFAHREN',
    loading: 'ERSTELLUNG...',
    package_name_order: 'Paket №4 «Premium»',
    errors: {
      not_found: 'Positives Ergebnis von Paket 1 erforderlich.',
      pending: 'Verfahren не завершено.',
      negative: 'Ergebnis nicht bestätigt.',
      generic: 'Fehler.',
    },
    btn_close: 'Schließen',
  },
  ua: {
    ...
    family: {
      title: 'СЕМЕЙНИЙ ФОРМАТ',
      spouse: 'Чоловік/Дружина',
      children: 'Діти',
      info: 'Базовий сімейний формат включає до 3 осіб. Кожен наступний учасник розраховується окремо.',
    },
    credits: {
      title: 'РАНІШЕ ПРИДБАНІ ПАКЕТИ (ЗАРАХУВАННЯ)',
      p1_label: 'Пакет №1 «Стартовий»',
      p2_label: 'Пакет №2 «Самостійний»',
      p3_label: 'Пакет №3 «Адвокатський»',
      p1_summary: 'Зарахування пакету №1',
      p2_summary: 'Зарахування пакету №2',
      p3_summary: 'Зарахування пакету №3',
    },
    summary: {
      base: 'БАЗОВА ЦІНА ПАКЕТУ №4',
    ...
    offer_scope:
      'Ця пропозиція призначена виключно для споживачів у розумінні § 13 BGB.',
    p1_precondition:
      'Бронювання цього пакета можливе лише за наявності позитивного результату в рамках Пакета №1.',
    privacy_link: 'Політика конфіденційності',
    button: 'ПЕРЕВІРИТИ ТА ПРОДОВЖИТИ',
    loading: 'СТВОРЕННЯ...',
    package_name_order: 'Пакет №4 «Преміум»',
    errors: {
      not_found: 'Потрібен позитивний результат Пакету №1.',
      pending: 'Оцінка не завершена.',
      negative: 'Результат не підтверджено.',
      generic: 'Помилка.',
    },
    btn_close: 'Закріти',
  },
...
export const P4PreCheckoutModal: React.FC<P4PreCheckoutModalProps> = ({
  onClose,
  onProceed,
}) => {
  ...
  const pricing = useMemo(
    () => PriceService.calculateUpgrade(4, selectedCredits, extraMembers),
    [selectedCredits, extraMembers]
  );
  ...
  const DocLink = ({ code, text }: { code: string; text: string }) => (
    <span
      ...
      className="text-blue-600 hover:text-blue-800 underline cursor-pointer hover:bg-blue-50 rounded px-1 transition-colors font-bold inline-flex items-center gap-1"
    >
      {text}
    </span>
  );
  ...
  const DownloadIcon = ({ code }: { code: string }) => (
    <button
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        alert(`Download: ${code}.pdf`);
      }}
      className="p-1 text-slate-400 hover:text-blue-600 transition-colors inline-block align-middle ml-0.5"
      title="PDF"
    >
      <FileDown size={14} />
    </button>
  ...
  const getCreditLabel = (amount: number) => {
    ...
    if (Math.abs(amount - 949) < 0.1) return t.credits.p3_summary;
    return 'Credit';
  };
          ...
          <div className="p-6 lg:p-8 overflow-y-auto custom-scrollbar space-y-6">
            ...
            <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <div className="space-y-2">
                {[
                  { id: 1, label: t.credits.p1_label, amount: 124.5 },
                  { id: 2, label: t.credits.p2_label, amount: 649 },
                  { id: 3, label: t.credits.p3_label, amount: 949 },
                ].map((credit) => (
                  <label
                    key={credit.id}
                    className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100 cursor-pointer hover:border-blue-900/30 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
            ...
            <div className="space-y-4">
              <div className="text-center px-4">
                <p className="text-[12px] font-bold text-blue-900 leading-relaxed uppercase tracking-tighter">
                  {t.offer_scope}
                </p>
              </div>

              <div
                className={`space-y-4 p-5 rounded-2xl border transition-all ${
                  showValidationErrors && !areLegalsValid
                    ? 'border-red-500 bg-red-50 animate-shake shadow-lg shadow-red-100'
                    : areLegalsValid
              ...
              >
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input
                    ...
                    className={`mt-1 w-5 h-5 rounded shrink-0 transition-all ${
                      showValidationErrors && !consentA
                        ? 'ring-2 ring-red-400 border-red-500'
                        : consentA
                        ? 'text-emerald-600 border-emerald-500'
                        : 'text-blue-900 border-slate-300'
                    }`}
                  />
                  <span
                    className={`text-[11px] font-medium leading-relaxed transition-colors ${
                      showValidationErrors && !consentA
                        ? 'text-red-700'
                        : consentA
                        ? 'text-emerald-700'
                  ...
                  >
                    {language === 'de' ? (
                      <>
                        Ich akzeptiere die <DocLink code="AGB" text="AGB" />{' '}
                        <DownloadIcon code="AGB" />, die{' '}
                        <DocLink code="MANDAT" text="Mandatsvereinbarung" />{' '}
                        <DownloadIcon code="MANDAT" /> sowie die{' '}
                        <DocLink
                          code="RVG"
                          text="Honorarvereinbarung (§ 3a RVG)"
                        />{' '}
                        <DownloadIcon code="RVG" />.
                      </>
                    ) : language === 'ua' ? (
                      <>
                        Я приймаю <DocLink code="AGB" text="AGB" />{' '}
                        <DownloadIcon code="AGB" />,{' '}
                        <DocLink code="MANDAT" text="Договір доручення" />{' '}
                        <DownloadIcon code="MANDAT" />, а також{' '}
                        <DocLink code="RVG" text="Гонорарну угоду" /> (§ 3a RVG){' '}
                        <DownloadIcon code="RVG" />.
                      </>
                    ) : (
                      <>
                        Я принимаю <DocLink code="AGB" text="AGB" />{' '}
                        <DownloadIcon code="AGB" />,{' '}
                        <DocLink code="MANDAT" text="Договор поручения" />{' '}
                        <DownloadIcon code="MANDAT" />, а также{' '}
                        <DocLink code="RVG" text="Гонорарное соглашение" /> (§
                        3a RVG) <DownloadIcon code="RVG" />.
                      </>
                    )}
                  </span>
                </label>
                  ...
                  >
                    {language === 'de' ? (
                      <>
                        ...
                        <DocLink
                          code="EXECUTION"
                          text="die sofortige Aufnahme der Tätigkeit"
                        />{' '}
                        <DownloadIcon code="EXECUTION" /> und weiß, dass mein{' '}
                        <DocLink code="WIDERRUF" text="Widerrufsrecht" /> gemäß
                        § 356 Abs. 4 BGB erlischt{' '}
                        <DownloadIcon code="WIDERRUF" />.
                      </>
                    ) : language === 'ua' ? (
                      <>
                        Я прямо запитую{' '}
                        <DocLink
                          code="EXECUTION"
                          text="негайний початок робіт"
                        />{' '}
                        <DownloadIcon code="EXECUTION" /> і знаю, що моє{' '}
                        <DocLink code="WIDERRUF" text="право на відкликання" />{' '}
                        припиняється відповідно до § 356 Abs. 4 BGB{' '}
                        <DownloadIcon code="WIDERRUF" />.
                      </>
                    ) : (
                      <>
                        ...
                        <DocLink
                          code="EXECUTION"
                          text="немедленное начало работ"
                        />{' '}
                        <DownloadIcon code="EXECUTION" /> и знаю, что моё{' '}
                        <DocLink code="WIDERRUF" text="право на отзыв" />{' '}
                        прекращается в соответствии с § 356 Abs. 4 BGB{' '}
                        <DownloadIcon code="WIDERRUF" />.
                      </>
                    )}
                  </span>
              ...
              <div className="text-center pt-2">
                <button
                  ...
                  className="text-[10px] font-bold text-blue-700 hover:text-blue-900 transition-colors uppercase tracking-widest underline"
                >
                  {t.privacy_link}
                </button>
              </div>
            ...
            {serverError && (
              <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-100 rounded-xl text-red-800 animate-in fade-in">
