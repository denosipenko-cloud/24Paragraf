...
interface P2PreCheckoutModalProps {
  onClose: () => void;
...
const modalContent = {
  ru: {
    ...
    subtitle: 'Пакет №2 «Самостоятельный»',
    placeholders: {
      name: 'ИМЯ *',
      name_ph: 'Имя',
      surname: 'ФАМИЛИЯ *',
    ...
    package_name_order: 'Пакет №2 «Самостоятельный»',
    errors: {
      not_found:
  ...
  de: {
    title: 'BESTELLUNG',
    subtitle: 'Paket №2 «Selbstständig»',
    placeholders: {
      name: 'VORNAME *',
      name_ph: 'Vorname',
      surname: 'NACHNAME *',
    ...
    package_name_order: 'Paket №2 «Selbstständig»',
    errors: {
      not_found:
  ...
  ua: {
    ...
    package_name_order: 'Пакет №2 «Самостійний»',
    errors: {
      not_found:
...
export const P2PreCheckoutModal: React.FC<P2PreCheckoutModalProps> = ({
  onClose,
  onProceed,
}) => {
  ...
  const pricing = useMemo(() => {
    // Включаем кредит за Пакет 1 автоматически, если галочка нажата
    const effectiveCredits = isP1Confirmed
      ? Array.from(new Set([...selectedCredits, 1]))
      : selectedCredits;
    // familyMembers учитываются
    return PriceService.calculateUpgrade(2, effectiveCredits, extraMembers);
  ...
  const handleProceed = async () => {
    setServerError(null);
    if (!isFormValid || !isP1Confirmed || !isDataConsent) {
      ...
      if (result.status === 'POSITIVE') {
        const orderDetails = {
          ...
          credits_applied: pricing.appliedCredits.map((c) => ({
            id: c.id,
            amount: c.amount,
            description: c.id === 1 ? 'Pack 1' : 'Credit',
          })),
        ...
        const payload = {
          ...
          description: `Клиент: ${formData.name} ${formData.surname}. Тел: ${fullPhone}`,
          metadata: {
            packageId: 'p2',
        ...
        else throw new Error('Ошибка создания ссылки оплаты.');
      } else {
        ...
        <div className="p-6 lg:p-8 overflow-y-auto custom-scrollbar space-y-6">
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={() => handleP1Confirmation(true)}
          ...
          <button
            ...
            className={`w-full h-14 rounded-full font-black text-[13px] uppercase tracking-widest shadow-xl flex items-center justify-center gap-3 transition-all ${
              isFormValid && isP1Confirmed && isDataConsent
