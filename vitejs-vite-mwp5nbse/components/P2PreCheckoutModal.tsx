...
export const P2PreCheckoutModal: React.FC<P2PreCheckoutModalProps> = ({ onClose, onProceed }) => {
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      if (result.status === 'POSITIVE') {
        const payload = { amount: pricing.finalTotal, email: formData.email, productName: t.package_name_order, metadata: { packageId: 'p2', clientName: `${formData.name} ${formData.surname}` } };
