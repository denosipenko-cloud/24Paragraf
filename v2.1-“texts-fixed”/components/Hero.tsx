...
import { Star, Scale, MapPin, Users, ArrowRight, Play, ExternalLink } from 'lucide-react';
...
export const Hero: React.FC<HeroProps> = ({ onCtaClick }) => {
  ...
  // Changed mute=1 to mute=0 to enable sound by default.
  // Since the iframe is rendered only after a user click, autoplay with sound is allowed by browsers.
            ...
            <div className="border-t border-slate-200 pt-6">
               <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-4 gap-x-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                     ...
                     <span className="text-[10px] text-slate-500 font-medium">{t('hero_trust_anwalt')}</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-0.5">
                     <div className="flex items-center gap-1.5 text-blue-900">
                        <Scale size={16} />
                        <span className="font-bold text-slate-900 text-sm">30+</span>
                     </div>
                     <span className="text-[10px] text-slate-500 font-medium">{t('hero_trust_exp')}</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-0.5">
                     ...
                     <span className="text-[10px] text-slate-500 font-medium">{t('hero_trust_won')}</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-0.5">
                     <div className="flex items-center gap-1.5 text-blue-900">
                        <MapPin size={16} />
                        <span className="font-bold text-slate-900 text-sm">München</span>
                     </div>
                     <span className="text-[10px] text-slate-500 font-medium">{t('hero_trust_office')}</span>
                  </div>
                  ...
                  <div className="relative bg-slate-900 rounded-2xl overflow-hidden shadow-xl aspect-video group cursor-pointer border border-slate-800">
                       {!isVideoPlaying ? (
                         <div 
                           ...
                           className="absolute inset-0 flex flex-col items-center justify-center transition-transform duration-300 z-10"
                         >
                            ...
                            <div className="relative mt-4 flex flex-col items-center">
                               <span className="text-white font-semibold text-xs tracking-wide uppercase">{t('hero_video_label')}</span>
                               <span className="text-slate-400 text-[10px] mt-0.5">{t('hero_video_sub')}</span>
                            </div>
                         </div>
                       ) : (
                         <iframe 
